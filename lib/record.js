'use strict';
import { Query, QueryCondition } from './conditions.js'
...
class GlideRecord {
  ...
  _parameters() {
    ...
    if(this.#currentIdx == -1)
      query_params['sysparm_offset'] = 0
    else
      query_params['sysparm_offset'] = this.#currentIdx + 1
    return query_params
  }
   ...
   * @param {*} condition 
   * @param {*} operator 
   * @returns {QueryCondition} the condition
   */
  ...
  getEncodedQuery() {
    return this.#query.generateQuery(this.#encodedQuery)
  }
